 ...
 * @param geneName Human gene name to fetch sequences for
 * @returns Promise resolving to record mapping species to aligned sequences
 */
export async function fetchCondensedSequences(geneName: string) {
  const res = await fetch(`${API_BASE_URL}/condensed_sequences/?gene_name=${encodeURIComponent(geneName)}`);
